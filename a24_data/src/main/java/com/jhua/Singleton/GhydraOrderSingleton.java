...
public class GhydraOrderSingleton {
    private static GhydraOrderSingleton instance = null;
    private static boolean hasOutPutXlsx = false;

    private static DataFrame<Object> raw_dataFrameOfGhydraOrder = null;
    private static DataFrame<Object> dataFrameOfGhydraOrder = null;

    private static String start;
    private static String end;

    private GhydraOrderSingleton() throws SQLException {

        //单例模式
        Connection connection = BaseDao.getConnection();
        try {
            ResultSet execute = null;
            String sql = SingletonUtil.sqlDate(Constants.GhydraOrder_SelectAll, start, end);
            System.out.println("单例模式-GhydraOrderSingleton：" + sql);
            execute = BaseDao.execute(connection, sql, null);
            raw_dataFrameOfGhydraOrder = DataFrame.readSql(execute);
...
//            );
//            DataDealer.writeXls(raw_dataFrameOfPlayRecord, Constants.OUT_PUT_SOURCE_PATH+"\\GhydraOrder.xlsx");
            raw_dataFrameOfGhydraOrder.writeCsv(Constants.OUT_PUT_SOURCE_PATH+"\\GhydraOrder.csv");
    ...
    //获取单例
    public static synchronized GhydraOrderSingleton getInstance() throws SQLException {
        if (instance == null) {
            instance = new GhydraOrderSingleton();
            System.out.println("单例模式-生成：GhydraOrderSingleton —— " + instance);
        }
        return instance;
    }
    ...
    public static DataFrame<Object> dataFrameOfGhydraOrder() throws IOException {
        try {
            getInstance();
            dataFrameOfGhydraOrder = UserOperationUtil.reduceUnnormalAccounts(raw_dataFrameOfGhydraOrder);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return dataFrameOfGhydraOrder;
    }
    ...
    //获取原始表
    public static DataFrame<Object> raw_dataFrameOfGhydraOrder() {
        try {
            getInstance();
            // 如果本次实例没有导出，那就导出
            if (!hasOutPutXlsx) {
                DataDealer.writeXls(raw_dataFrameOfGhydraOrder, Constants.RESOURCE_PATH+"/raw/user.xlsx");
                hasOutPutXlsx = true;
            }
        } catch (SQLException | IOException throwables) {
            throwables.printStackTrace();
        }
        return raw_dataFrameOfGhydraOrder;
    }
}
